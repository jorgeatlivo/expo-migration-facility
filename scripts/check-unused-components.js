...
function findUnusedComponents() {
  ...
  const outputPath = path.join(__dirname, "../unused-components.md");
  let outputContent = "# Unused Components\n\n";
  outputContent += `*Generated on ${new Date().toLocaleString()}*\n\n`;
  outputContent += `Total components found: ${components.length}\n\n`;

  if (duplicateComponents.length) {
    outputContent += "## Components with duplicate names\n\n";
    duplicateComponents.forEach(dup => {
      outputContent += `- **${dup.name}**: Found in ${dup.paths.length} files\n`;
      dup.paths.forEach(p => outputContent += `  - ${p}\n`);
    });
    outputContent += "\n> Warning: Components with the same name may cause false negatives in usage detection.\n\n";
  }

  outputContent += "## Unused Components\n\n";
  if (unusedComponents.length) {
    unusedComponents.forEach(component => {
      const relativePath = path.relative(COMPONENTS_DIR, component);
      outputContent += `- \`${relativePath}\`\n`;
    });
  } else {
    outputContent += "None found.\n";
  }
