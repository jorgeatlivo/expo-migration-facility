...
import { ProfessionalAgenda, ProfessionalAgendaDetailView } from "../types/professionalAgenda"
...
export const fetchProfessionalAgenda = async (
   ...
   const uri = date && shiftTimeInDay ? `/facility/professionals-agenda?date=${date}&shiftTimeInDay=${shiftTimeInDay}` : '/facility/professionals-agenda';
   return api
   .get(uri)
   .then(response => {
